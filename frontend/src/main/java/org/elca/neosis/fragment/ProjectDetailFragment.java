...
public class ProjectDetailFragment {
    ...
    private void initMultilingual() {
        ...
        fragmentProjectNumberInputLabel.textProperty().bind(I18N.createStringBinding(
                ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_PROJECT_NUMBER_INPUT_TITLE));
        fragmentProjectNameInputLabel.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_PROJECT_NAME_INPUT));
        fragmentCustomerInputLabel.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_CUSTOMER_INPUT));
        fragmentGroupInputLabel.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_GROUP_INPUT));
        fragmentMembersInputLabel.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_MEMBERS_INPUT));
        fragmentStatusInputLabel.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_STATUS_INPUT));
        fragmentStartDateInputLabel.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_START_DATE_INPUT));
        fragmentEndDateInputLabel.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_END_DATE_INPUT));
        projectVisaNotValidMessage.textProperty().bind(I18N.createStringBinding(
                ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_EMPLOYEE_VISAS_NOT_EXISTED));
        missingRequiredFieldsMessage.textProperty().bind(I18N.createStringBinding(
                ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_MISSING_REQUIRED_FIELDS));
        projectNumberExistedMessage.textProperty().bind(I18N.createStringBinding(
                ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_PROJECT_NUMBER_EXISTED));

        // Init multilingual for buttons
        cancelButton.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_CANCEL_BUTTON));
    ...
    private void initMultilingualCreateProject() {
        ...
        submitButton.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_CREATE_BUTTON));
        fragmentTitle.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_CREATE_TITLE));
    }

    private void initMultilingualEditProject() {
        initMultilingual();
        submitButton.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_EDIT_BUTTON));
        fragmentTitle.textProperty()
                .bind(I18N.createStringBinding(ApplicationBundleKey.LABEL_PROJECT_DETAIL_FRAGMENT_EDIT_TITLE));
    }
    ...
    private void handleCreateProject() {
        ...
        if (!requiredEmptyTextFields.isEmpty()) {
            ...
            ConfirmationAlert confirmDialog = new ConfirmationAlert(
                    ...
                    I18N.get(ApplicationBundleKey.BUTTON_CANCEL_CREATE_ALERT), Alert.AlertType.CONFIRMATION);
            ...
            if (confirmed) {
                ...
                Set<String> members = Arrays.stream(membersInput.getText().split(MEMBER_SEPARATOR_SYMBOL))
                        .map(String::trim).filter(visa -> !visa.isEmpty()).collect(Collectors.toSet());
                NewProject request = NewProject.newBuilder()
                        ...
                        .addAllMembers(members).setStartDate(
                                startDateInput.getValue().format(DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_PATTER)))
                        .setEndDate(!Objects.isNull(endDate) ? endDate.format(
                                DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_PATTER)) : "").build();
                try {
    ...
    private void handleEditProject(Project project) {
        ...
        if (!requiredEmptyTextFields.isEmpty()) {
            ...
            ConfirmationAlert confirmDialog = new ConfirmationAlert(
                    ...
                    I18N.get(ApplicationBundleKey.BUTTON_CANCEL_UPDATE_ALERT), Alert.AlertType.CONFIRMATION);
            boolean confirmed = confirmDialog.showConfirmationDialog();
            ...
            if (confirmed) {
                ...
                Set<String> members = Arrays.stream(membersInput.getText().split(MEMBER_SEPARATOR_SYMBOL))
                        .map(String::trim).filter(visa -> !visa.isEmpty()).collect(Collectors.toSet());
                Project request = Project.newBuilder().setId(project.getId()).setVersion(project.getVersion())
                        .setNumber(Integer.parseInt(projectNumberInput.getText())).setName(projectNameInput.getText())
                        .setCustomer(customerInput.getText()).setGroupId(groupInput.getValue())
                        .setStatus(ApplicationMapper.convertToProjectStatus(statusInput.getValue()))
                        .addAllMembers(members).setStartDate(
                                startDateInput.getValue().format(DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_PATTER)))
                        .setEndDate(!Objects.isNull(endDate) ? endDate.format(
    ...
    public void init(Integer projectNumber) {
        ...
        statusInput.valueProperty().addListener(
                ((observable, oldValue, newValue) -> currentSelectedStatusIndex = statusInput.getSelectionModel()
                        .getSelectedIndex()));
    }
        ...
        try {
            ...
            if (response.getIsExisted()) {
                ...
                startDateInput.setValue(
                        LocalDate.parse(response.getStartDate(), DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_PATTER)));
                endDateInput.setValue(
                        StringUtils.isEmpty(response.getEndDate()) ? null : LocalDate.parse(response.getEndDate(),
                                DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_PATTER)));
    ...
    private void initProjectStatus() {
        I18N.localeProperty().addListener(((observable, oldValue, newValue) -> {
            statusInput.getItems().setAll(I18N.get(ApplicationBundleKey.PROJECT_NEW_STATUS_KEY),
    ...
    private void initDatePickerFormatter() {
        ...
        startDateInput.setConverter(
                new LocalDateStringConverter(DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_PATTER), null));
        endDateInput.setConverter(
                new LocalDateStringConverter(DateTimeFormatter.ofPattern(DATE_TIME_FORMAT_PATTER), null));
    }
    ...
    private void hideErrorMessage() {
        ...
        startDateAfterEndDateErrors.setVisible(false);
    }
